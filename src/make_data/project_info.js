export const project_info = [
  {
    project_id: "c733ad20-63bf-4c00-b856-c018b0a9b094",
    info: {
      tracking_modes: {
        image: {
          tracks: [
            {
              scenes: [
                {
                  assets: [
                    {
                      src: "https://supabase.wemear.com/storage/v1/object/public/assets/images/90x140Sri1.jpg",
                      type: "Image",
                      scale: [0.5, 0.5, 0.5],
                      opacity: 1,
                      asset_id: "asset_2f97f79d-963f-47a1-9a67-85a61ce052fc",
                      position: [
                        0.7946999735959775,
                        -0.000000000000000006938893903907228,
                        0.0000000000000008881784197001252,
                      ],
                      rotation: [0, 0, 0],
                      asset_name: "90x140Sri1.jpg",
                    },
                    {
                      src: "https://supabase.wemear.com/storage/v1/object/public/assets/videos/Professional_Mode_Generated_Video (8).mp4",
                      type: "Video",
                      scale: [0.5, 0.5, 0.5],
                      opacity: 1,
                      asset_id: "asset_6748a1ff-b7f7-4107-8e27-0e8e16d03077",
                      position: [
                        -0.6182467746004701, -0.08829288873206753,
                        -0.000000000000018692297919179402,
                      ],
                      rotation: [0, 0, 0],
                      asset_name: "Professional_Mode_Generated_Video (8).mp4",
                    },
                    {
                      src: "https://supabase.wemear.com/storage/v1/object/public/assets/3d/DEEbase_basic_pbr.glb",
                      type: "3D Model",
                      scale: [0.5, 0.5, 0.5],
                      opacity: 1,
                      asset_id: "asset_6fbf4fbf-f0c7-4f39-ab29-63737ff82b2c",
                      position: [0.008934980348556776, 0.14998761218649137, 0],
                      rotation: [0, 0, 0],
                      asset_name: "DEEbase_basic_pbr.glb",
                    },
                    {
                      src: "https://supabase.wemear.com/storage/v1/object/public/assets/3d/men2025.glb",
                      type: "3D Model",
                      scale: [0.5, 0.5, 0.5],
                      opacity: 1,
                      asset_id: "asset_b6525015-dbf9-4f45-b6eb-32451d4fa1dc",
                      position: [0, 0, 0],
                      rotation: [0, 0, 0],
                      asset_name: "men2025.glb",
                    },
                  ],
                  scene_id: "S1",
                },
              ],
              track_id: "T1",
            },
          ],
          mindFile: {
            mind_id: "99cebd43-f658-414f-ad19-44bf388aa050",
            mind_src:
              "https://supabase.wemear.com/storage/v1/object/public/assets/mind/Test_2",
            mind_name: "Test_2",
            mind_image: {
              T1: "https://supabase.wemear.com/storage/v1/object/public/assets/mind/images/Test_2_T1",
            },
          },
          setting:{
            background:"./background_default.jpg",
            icon:"./icon_default.jpg",
          }
        },
      },
    },
  },
];
